...
const PostPage = () => {
  ...
  /* const loadComments = async ( ) => {
    const newComments = await getComment(id as string, page, limit)

    setComments((prev) => [...prev, ...newComments])

    if (newComments.length < limit) setLoadMore(false)
 ...
 */
  ...
  const fetchComments = async () => {
    try {
      const response = await getComment(id as string, page, limit);
      console.log("Comment response:", response);

      if (Array.isArray(response)) {
        console.log("Total comments received", response.length);
        setComments((prev) => [...prev, ...response]);

        if (response.length < limit) setLoadMore(false);
      } else {
