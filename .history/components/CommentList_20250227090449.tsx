...
const CommentList = ({ comments, setComments, postId }: CommentProps) => {
  ...
  const handleAddReply = async (parentCommentId: string, postId: string) => {
    if (!replyText.trim()) return;
    const response = await addReply(parentCommentId, postId, replyText);
          ...
          <div key={comment._id}>
            ...
            {showReply === comment._id && (
              <div className="mt-2 flex items-center gap-2 mb-5">
                ...
                />
                <Button onClick={() => handleAddReply(postId, comment._id)}>
                  Reply
                </Button>
              </div>
            )}
