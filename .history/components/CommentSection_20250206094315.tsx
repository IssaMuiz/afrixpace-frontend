...
const CommentSection = () => {
  ...
  function Comment({
    comment, onReply, replyingTo, setReplyText, replyText, handleAddReply,
  }: {
    comment: CommentType;
    onReply: () => void; 
    ...
    return (
      <div className="p-3 border rounded-md bg-gray-100">
        <div className="flex items-center gap-2 mb-2"> <Avatar><AvatarImage src={comment.avatar}/>
        <AvatarFallback>{comment.author[0]}</AvatarFallback>
        </Avatar>
        <span className="font-semibold">{comment.author}</span>
        </div>
        <p className="text-sm text-gray-700">{comment.text}</p>
        <button className="text-blue-600 text-xs mt-2 hover:underline" onClick={onReply}>Reply</button>
        ...
        {comment.replies && comment.replies.length > 0 && (
          <div className="mt-2 pl-6 border-l-2 border-gray-300">{comment.replies.map((reply) => (<Comment key={reply.id} comment={reply} onReply={() => {}} replyingTo={null} setReplyText={() => {}} replyText="" handleAddReply={() => {}}/>))}</div> 
        )}
      </div>
    )
  }
